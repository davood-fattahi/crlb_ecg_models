function [P, Q, R, S, T] = ecgWavesSoI_nrmlBeat(ecg, Rpeak, RRf)
...
for i= 1: length(RR)
    ...
    [~,I]=min(ecg(Rpeak(i+1)-floor(.1*RR(i)):Rpeak(i+1))); Q(i+1,2)=I+Q(i+1,1)-1;
    [~,I]=min(abs(ecg(Q(i+1,2):Rpeak(i+1)))); Q(i+1,3)=I+Q(i+1,2);
    [~,I]=min(abs(ecg(Rpeak(i+1)-floor(.1*RR(i)):Q(i+1,2)))); Q(i+1,1)=I+Rpeak(i+1)-floor(.1*RR(i))-1;
    
    if (Q(i+1,3)==Q(i+1,1))
%     Q(i+1,3)=Q(i+1,2)+floor(.01*RR(i));
    Q(i+1,3)=Q(i+1,2)+round(.4*(Rpeak(i+1)-Q(i+1,2)));
    end    
    
    S(i,3)=Rpeak(i)+floor(.15*(RR(i)));
    S(i,1)=Rpeak(i)+floor(.02*(RR(i))); 
