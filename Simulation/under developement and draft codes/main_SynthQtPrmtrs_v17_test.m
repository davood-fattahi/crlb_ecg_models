...
PostNumBeats=100; % max number of the beats envolved in noisy signals' parameter estimation stage
NumRuns=1; % number of repeats in noisy signals' parameter estimation stage
PriNumBeats=1000; % max number of the beats envolved in prior parameter estimation stage (clean signals)

%%% option structure for non linear least squre optimization, respectively
%%% in ML estimation and Bayesian estimation:
% options = struct('Algorithm', 'levenberg-marquardt','SpecifyObjectiveGradient',true);
...
beta=.4;
PrQtInt=PrParamsT(:,3) + beta*PrParamsT(:,2) - PrParamsQ(:,3) + beta*PrParamsQ(:,2);
MlParamsQtInt=squeeze(MlParamsT(3,:,:,:) + beta*MlParamsT(2,:,:,:) - MlParamsQ(3,:,:,:) + beta*MlParamsQ(2,:,:,:));
BysParamsQtInt=squeeze(BysParamsT(3,:,:,:) + beta*BysParamsT(2,:,:,:) - BysParamsQ(3,:,:,:) + beta*BysParamsQ(2,:,:,:));
MlErQtInt=MlParamsQtInt-PrQtInt(JJ); % ML Error from prior
BysErQtInt=BysParamsQtInt-PrQtInt(JJ); % Bys Error from prior
...
xlabel 'SNR (dB)'; ylabel 'Error varinace (Sec^2)';
legend('ML MSE','ML CRLB',' BYS MSE','BYS CRLB')
% title 'Determinants of Error Cov. and CRLB matrices for Q wave Gaus. parameters'
saveas(gcf,'Backup and Results\QTint_SynthData.fig')
saveas(gcf,'Backup and Results\QTint_SynthData.eps','epsc')
