...
%     signal=signal-(BaseLine1(BaseLine1(signal', round(w1*SynthEcgFs), 'md'), round(w2*SynthEcgFs), 'mn'))';
    ...
    [~, Q, R, ~, T] = ecgWavesSoI2(signal, Rpeaks, SynthEcgFs, 1);
...
%     [~, Q, R, ~, T] = ecgWavesSoI_nrmlBeat(signal, Rpeaks, 1);
    ...
    Q=Q(I,:); R=R(I,:); T=T(I,:); Rpeaks=Rpeaks(I); NumBeats=length(Rpeaks);

    Rp(end+1:end+NumBeats-2,:) = R(2:end-1,2)+floor(.4*SynthEcgFs)+1;
%     RSegOff(end+1:end+NumBeats-2,:) = RR(2:end-1,3)+floor(.4*SynthEcgFs)+1;
    
    QSegOn(end+1:end+NumBeats-2,:) = Q(2:end-1,1)+floor(.4*SynthEcgFs)+1;
    Qp(end+1:end+NumBeats-2,:) = Q(2:end-1,2)+floor(.4*SynthEcgFs)+1;
    QSegOff(end+1:end+NumBeats-2,:) = Q(2:end-1,3)+floor(.4*SynthEcgFs)+1;
    
    TSegOn(end+1:end+NumBeats-2,:) = T(2:end-1,2)- fix(0.02*SynthEcgFs) + floor(.4*SynthEcgFs)+1;
    Tp(end+1:end+NumBeats-2,:) = T(2:end-1,2)+floor(.4*SynthEcgFs)+1;
    TSegOff(end+1:end+NumBeats-2,:) = T(2:end-1,3) + floor(.4*SynthEcgFs)+1;
