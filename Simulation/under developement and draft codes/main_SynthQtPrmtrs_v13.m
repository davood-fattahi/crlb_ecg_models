...
nc=20;
Chnl = (1:3); % ECG channel used as the source of resempling
phase0 = 0;   % desired phase shift
bins = 250;   % number of phase bins
SynthEcgFs=500;
pstd=.04;
SynthSigLength=1*60*60; % length of synthetic ecg (sec)
HrTr= 70/60; % hertz, threshold for heart rate, used in r peak detector
...
Qpub='[0 (Qt(j,3)-Qt(j,1))/4 Qt(j,3)]'; % optimization conditions: upper bound for the Q wave parameters
...
oldFolder=cd('..\mcode');
AdrsNm=[DA  '\' NC{nc}];
[ecg,fs,~]=rdsamp(AdrsNm,[],N,N0); % 2->ii, 8->v2, 11->v5
cd(oldFolder)
...
[signal, Rp]=ECGResembler(tm, [], pstd, [], [], 0, ecg, fs, HrTr/fs, bins);
signal=signal(:);
...
% Rp = PeakDetection(signal(:),HrTr/SynthEcgFs); 
Rp=find(Rp);
...
for j=1:NumAllBeats  % for each beat ...
    ...
    p0=[signal(Q(j,2)) (Qt(j,3)-Qt(j,1))/8 Qt(j,2)]; % initial condition
...
rng(1) % fixing the seed for random permuting
...
for k=1:length(SNR) % for each SNR
    ...
    for m=1:NumRuns % for each run
        ...
        for j=JJ
            ...
            tt=tm(QSegOn(j):QSegOff(j))-Rt(j,2); % SOI time stamp
            p0=[signal(Q(j,2)) (Qt(j,3)-Qt(j,1))/8 Qt(j,2)]; % initial points
...
for k=1:length(SNRdB)
    for j=1:NumAllBeats
        ...
        [~, MlFIApprQ(:,:,j,k)]=GaussCRLB(PrParamsQ(j,1), PrParamsQ(j,2), SynthEcgFs, VarNoiseAddQ(k) );
        [~, MlFIApprT(:,:,j,k)]=GaussCRLB(PrParamsT(j,1), PrParamsT(j,2), SynthEcgFs, VarNoiseAddT(k) );
...
save(['Backup and Results\TempSynthQtPrmtrs_' num2str(nc)  '_3.mat']);
% 
% figure
