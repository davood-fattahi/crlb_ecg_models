...
Qplb='[-1   0.1*(tm(QSegOff(j))-tm(QSegOn(j))) tm(QSegOn(j))    0  0.1*(tm(RSegOff(j))-tm(QSegOff(j))) tm(QSegOff(j))]'; % optimization conditions: lower bound for the Q wave parameters
Qpub='[0    2*(tm(QSegOff(j))-tm(QSegOn(j)))   tm(QSegOff(j))   5  2*(tm(RSegOff(j))-tm(QSegOff(j))) tm(RSegOff(j))]'; % optimization conditions: upper bound for the Q wave parameters

Tplb='[-5  0  0]';% optimization conditions: lower bound for the T wave parameters
Tpub='[5   10*(tm(TSegOff(j))-tm(TSegOn(j))) tm(TSegOff(j))]';% optimization conditions: upper bound for the T wave parameters
...
for j=1:PriNumBeats  % for each beat ...
    ...
    p0=[AllBeats(j,Qp(j)) (tm(RSegOff(j))-tm(QSegOn(j)))/8 tm(Qp(j)) ...
        AllBeats(j,Rp(j)) (tm(RSegOff(j))-tm(QSegOn(j)))/8 tm(Rp(j)) ]; % initial condition
