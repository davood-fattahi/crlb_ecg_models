...
for i= 1:NumCases
    ...
    Rp = PeakDetection(mean(signal,2),HrTr/Fs); Rp=find(Rp);
...
%     MlCrlbNumT=zeros(3, 3, NumAllBeats, length(SNRdB));
    ...
    for k=1:length(SNRdB)
        for j=1:NumAllBeats
            [~, MlFINumQ(:,:,j,k)]=GaussCrlbNumeric(tm(QSegOn(j):QSegOff(j))-Rpt(j), PrParamsQ(j,:), VarNoiseAddQ(k), 1 );
            [~, MlFINumT(:,:,j,k)]=GaussCrlbNumeric(tm(TSegOn(j):TSegOff(j))-Rpt(j), PrParamsT(j,:), VarNoiseAddT(k), 1 );
